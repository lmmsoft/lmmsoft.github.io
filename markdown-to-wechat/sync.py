...
class NewClient:
    ...
    def __real_get_access_token(self):
        postUrl = f"https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={APP_ID}&secret={APP_SECRET}"
...
def get_images_from_markdown(content):
    ...
    for line in lines:
        line = line.strip()
        if line.startswith('![') and line.endswith(')'):  # 只处理 ![xxx](xxx) 这种格式开头结尾的行
...
def gen_css(path, *args):
    tmpl = open(f"./assets/{path}.tmpl", "r").read()
...
def replace_links(content):
    pq = PyQuery(open(TEMP_HTML).read())
    links = pq('a')
    refs = []
    index = 1
    if len(links) == 0:
...
def upload_media_news(post_path):
    ...
    if len(images) == 0 or gen_cover == "true":
        ...
        print(seed)
        images = [f"https://picsum.photos/seed/{seed}/400/600"] + images
    uploaded_images = {}
    for image in images:
        media_id = ''
        media_url = ''
    ...
    CONTENT_SOURCE_URL = 'https://catcoding.me/p/{}'.format(link)  # todo
...
def run(string_date):
    ...
    print(string_date)
    pathlist = Path(POST_PATH).glob(POST_FORMAT)
...
if __name__ == '__main__':
    ...
    times = [datetime.now(), datetime.now() - timedelta(days=1)]  # 处理x天内的文章
    for x in times:
